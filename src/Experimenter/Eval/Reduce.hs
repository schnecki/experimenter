...
reduceUnaryOf Last {} (EvalVector tp unit vals) =
 ...
 (EvalValue _ _ _ _ v:_)    -> force $! EvalReducedValue (Id $ Last $ Stats tp) UnitScalar ((^?! evalY) $ last vals)
 (EvalReducedValue _ _ v:_) -> force $! EvalReducedValue (Id $ Last $ Stats tp) UnitScalar ((^?! evalValue) $ last vals)
...
reduceUnaryOf (EveryXthElem nr _) (EvalVector tp unit vals) =
  case vals of
    ...
    (EvalValue _ _ _ _ v:_)    -> force $! EvalVector (Id $ EveryXthElem nr $ Stats tp) unit (extractEvery nr vals)
    (EvalReducedValue _ _ v:_) -> force $! EvalVector (Id $ EveryXthElem nr $ Stats tp) unit (extractEvery nr vals)
    [] -> error "empty elements in reduceUnaryOf EveryXthElem{}"
  where
    extractEvery m = map snd . filter (\(x, _) -> mod x m == 0) . zip [1 ..]
reduceUnaryOf Length {} (EvalVector tp unit vals) = force $! EvalReducedValue (Id $ Length $ Stats tp) UnitScalar (fromIntegral $ length vals)
